...
def test_next_open_shadow_validation_skips_broker_readiness_in_brokerless_mode(monkeypatch, capsys, tmp_path) -> None:
...
def test_next_open_shadow_validation_honors_trade_date_override(monkeypatch, capsys, tmp_path) -> None:
    script = Path(__file__).resolve().parents[1] / "scripts" / "run_next_open_shadow_validation.py"
    spec = importlib.util.spec_from_file_location("run_next_open_shadow_validation_override", script)
    module = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    ...
    def fake_run_step(name, command, *, timeout, env):
        return {
            ...
            "returncode": 0,
            "timeout_seconds": timeout,
            "started_at_utc": "2026-05-28T22:30:00+00:00",
            "finished_at_utc": "2026-05-28T22:30:01+00:00",
            "stdout_tail": "",
            "stderr_tail": "",
        }
    ...
    assert '"trade_date": "2026-05-29"' in payload
    assert (tmp_path / "reports" / "live_monitor" / "next_open_shadow_validation_2026-05-29.json").exists()
