...
set +e
"$PYTHON" "$ROOT/scripts/run_next_open_shadow_validation.py" "$@" > "$VALIDATION_LOG" 2>&1
STATUS=$?
set -e

if [[ "${ROI_SNIPS_SEND_MORNING_UPDATE:-false}" == "true" ]]; then
  UPDATE_LOG="$LOG_DIR/update_dispatch_${STAMP}.log"
  "$PYTHON" "$ROOT/scripts/send_morning_update.py" \
    --date "$(date +%F)" \
    --send \
    --account "${ROI_SNIPS_UPDATE_ACCOUNT:-roisnips}" \
    --target "${ROI_SNIPS_UPDATE_TARGET:-8262254077}" \
    > "$UPDATE_LOG" 2>&1 || true
fi
